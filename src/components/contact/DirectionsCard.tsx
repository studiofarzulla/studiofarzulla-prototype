...
import { IoAirplane, IoCarSport, IoTrain, IoBus, IoTime, IoLocation, IoCall } from 'react-icons/io5';
    ...
    steps: [
      {
        ...
        duration: '5 min',
        distance: '0.5 km',
        description: 'Exit the arrivals hall and head to the transportation area',
        tips: [
      ...
      },
      {
        ...
        duration: '40 min',
        distance: '34.5 km',
        description: 'Take Heydar Aliyev Avenue towards city center, then coastal road',
        tips: [
  ...
  return (
    <div className={cn('space-y-8', className)}>
      {DIRECTION_ROUTES.map((route, routeIndex) => (
        ...
        >
          {/* Route Header */}
          <div className={cn('px-6 py-4 border-b border-gray-100', getColorClasses(route.color, 'bg'))}>
            <div className="flex items-center gap-3">
              <div className={cn('w-12 h-12 rounded-lg flex items-center justify-center bg-white shadow-sm', getColorClasses(route.color, 'text'))}>
                <route.icon className="w-6 h-6" />
              </div>
              <div>
                <h3 className={cn('text-xl font-bold', getColorClasses(route.color, 'text'))}>
                  {route.title}
                </h3>
                <p className={cn('text-sm opacity-80', getColorClasses(route.color, 'text'))}>
                  {route.subtitle}
                </p>
              </div>
            </div>
              ...
              {route.steps.map((step, stepIndex) => (
                ...
                >
                  ...
                  {stepIndex < route.steps.length - 1 && (
                    <div className={cn('absolute left-6 top-12 w-0.5 h-16 -z-10', getColorClasses(route.color, 'bg'))} />
                  )}

                  <div className="flex gap-4">
                    {/* Step Icon */}
                    <div className={cn('w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0', getColorClasses(route.color, 'bg'), getColorClasses(route.color, 'text'))}>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {step.cost && (
                            <span className={cn('px-2 py-1 rounded-full text-sm', getColorClasses(route.color, 'bg'), getColorClasses(route.color, 'text'))}>
                              {step.cost}
                            </span>
                          )}
                        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-sm text-white/90">
              Complimentary airport shuttle service available with advance reservation (24 hours notice required).
            </p>
          </div>
          <div>
            <h4 className="font-semibold mb-2">🚗 Valet Parking</h4>
            <p className="text-sm text-white/90">
              Complimentary valet parking service for all hotel guests. Self-parking also available.
            </p>
          </div>
          <div>
            <h4 className="font-semibold mb-2">🚕 Taxi Service</h4>
            <p className="text-sm text-white/90">
              Our concierge can arrange reliable taxi services and provide estimated fares for popular destinations.
            </p>
          </div>
          <div>
            <h4 className="font-semibold mb-2">🚌 Public Transport</h4>
            <p className="text-sm text-white/90">
              Baku has an excellent metro system and bus network. Our staff can provide routes and schedules.
            </p>
          </div>
        </div>
      </div>
