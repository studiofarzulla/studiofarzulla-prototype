...
export default function BuildingSelector({ selectedBuilding, onBuildingSelect, className = '' }: BuildingSelectorProps) {
    ...
    <div className={`space-y-6 ${className}`}>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {buildings.filter(b => b.type === 'standard').map((building, index) => (
            <motion.div
              key={building.id}
            ...
            >
              ...
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="text-right">
                  <div className="text-lg font-bold text-primary-600">{building.rooms}</div>
              ...
              <div className="flex items-center justify-between mb-3">
                <span className={`text-sm font-medium ${getAvailabilityColor(building.available, building.rooms)}`}>
              ...
              <div className="mb-3">
                <span className="text-sm text-gray-600">
                  ${building.priceRange[0]} - ${building.priceRange[1]} / night
                </span>
              </div>
              ...
              <div className="space-y-2">
                ...
                {building.features.length > 2 && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setShowDetails(showDetails === building.id ? null : building.id);
                    }}
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {buildings.filter(b => b.type === 'corporate').map((building, index) => (
            <motion.div
              key={building.id}
              initial={{ y: 20, opacity: 0 }}
              animate={{ y: 0, opacity: 1 }}
            ...
            >
              ...
              <div className="flex justify-between items-start mb-4">
                ...
                <div className="text-right">
                  <div className="text-xl font-bold text-accent-600">{building.rooms}</div>
                  <div className="text-xs text-gray-500">rooms</div>
                </div>
              </div>
              ...
              <div className="bg-accent-100 p-3 rounded-lg mb-4">
                <div className="flex items-center text-accent-800">
                  ...
                  <span className="text-sm font-medium">Full Building Rental Available</span>
                </div>
                <div className="text-sm text-accent-700 mt-1">
                  Perfect for corporate events, training programs, and team retreats
                </div>
              </div>
              ...
              <div className="mb-4">
                <span className="text-sm text-gray-600">
                  ${building.priceRange[0]} - ${building.priceRange[1]} / night (full building)
                </span>
              </div>
          ...
          {(() => {
            ...
            return building ? (
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                <div>
                  <div className="font-medium text-primary-900">{building.name}</div>
                  <div className="text-primary-700">{building.location}</div>
                </div>
                <div>
                  <div className="font-medium text-primary-900">Capacity</div>
                  <div className="text-primary-700">{building.rooms} rooms total</div>
                </div>
                <div>
                  <div className="font-medium text-primary-900">Availability</div>
                  <div className={`${getAvailabilityColor(building.available, building.rooms)}`}>
