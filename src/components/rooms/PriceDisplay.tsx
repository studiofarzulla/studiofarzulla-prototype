...
export default function PriceDisplay({ 
  ...
  const [selectedSeason, setSelectedSeason] = useState<'low' | 'mid' | 'high' | 'peak'>('mid');
  ...
  const specialOffers: SpecialOffer[] = [
    {
      ...
      conditions: ['Minimum 3 nights', 'Non-refundable', 'Valid for standard rooms']
    },
    {
      id: 'extended-stay',
      title: 'Extended Stay',
      ...
      conditions: ['Minimum 7 nights', 'All room types', 'Flexible cancellation']
    },
    {
      id: 'corporate-rate',
      ...
      conditions: ['Corporate verification required', 'Bulk booking discounts', 'Saipem & BP preferred rates']
    }
  ];
  ...
  useEffect(() => {
    ...
    const filtered = specialOffers.filter(offer => {
      if (offer.id === 'corporate-rate' && roomType !== 'corporate') return false;
  ...
  const calculateViewSupplement = () => {
    switch (viewType) {
      case 'sea': return basePrice * 0.2;
      case 'pool': return basePrice * 0.1;
  ...
  const calculateRoomTypeSupplement = () => {
    switch (roomType) {
      case 'deluxe': return basePrice * 0.3;
      case 'suite': return basePrice * 0.8;
      case 'family': return basePrice * 0.4;
      case 'corporate': return basePrice * 1.2;
      default: return 0;
    }
  };
  ...
  const getCurrentPrice = () => {
    ...
    return (basePrice + viewSupplement + roomTypeSupplement) * pricing.multiplier;
  };
        ...
        <div className="text-center mb-6">
          ...
          {selectedSeason !== 'mid' && (
            <div className="flex items-center justify-center mt-2">
              ...
              <span className={`text-sm ${getCurrentPricing().multiplier < 1 ? 'text-green-600' : 'text-red-600'}`}>
                {getCurrentPricing().multiplier < 1 ? 'Seasonal Discount' : 'Peak Season'}
              </span>
            </div>
          )}
        </div>
            ...
            <div className="space-y-2 text-sm">
              ...
              {calculateViewSupplement() !== 0 && (
                <div className="flex justify-between">
                  ...
                  <span className={calculateViewSupplement() > 0 ? '' : 'text-green-600'}>
                    {calculateViewSupplement() > 0 ? '+' : ''}{formatPrice(calculateViewSupplement())}
                  </span>
                </div>
              )}
              ...
              <div className="flex justify-between">
                ...
                <span className={getCurrentPricing().multiplier > 1 ? 'text-red-600' : getCurrentPricing().multiplier < 1 ? 'text-green-600' : ''}>
        ...
        <div className="bg-gradient-to-r from-accent-50 to-primary-50 rounded-2xl p-6 border border-accent-200">
          ...
          <div className="space-y-4">
            {availableOffers.map((offer) => (
              <div key={offer.id} className="bg-white rounded-lg p-4 border border-accent-200">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center">
                    <div className="bg-accent-600 text-white px-2 py-1 rounded text-sm font-medium">
                      -{offer.discount}%
                    </div>
                    <h4 className="font-semibold text-gray-900 ml-3">{offer.title}</h4>
                  </div>
        ...
        <div className="bg-white rounded-xl p-6 border border-gray-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              ...
              <div key={stay.nights} className="text-center p-4 bg-primary-50 rounded-lg border border-primary-200">
                <div className="text-2xl font-bold text-primary-600">{stay.nights}</div>
                <div className="text-sm text-gray-600 mb-2">{stay.label}</div>
                <div className="text-lg font-semibold text-green-600">
                  -{stay.discount}%
                </div>
                <div className="text-sm text-gray-500">
                  {formatPrice(getCurrentPrice() * stay.nights * (1 - stay.discount / 100))}
                </div>
      ...
      {roomType === 'corporate' && (
        <div className="bg-gradient-to-r from-gray-50 to-blue-50 rounded-xl p-6 border border-blue-200">
          ...
          <p className="text-gray-600 mb-4">
            Special partnership rates available for Saipem, BP, and other oil industry partners. 
            Contact our corporate sales team for customized packages.
          </p>
