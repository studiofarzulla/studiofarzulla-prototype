  ...
  return (
    ...
    >
      ...
      <div className="relative h-64 overflow-hidden">
        ...
        {/* Room Type Badge */}
        <div className={`absolute top-4 left-4 px-3 py-1 rounded-full text-sm font-medium ${getRoomTypeColor()}`}>
          {t(`filter_${roomType}`)}
        </div>
      ...
      <div className="p-6">
        ...
        <div className="flex items-center space-x-4 mb-4 text-gray-600">
          <div className="flex items-center space-x-1">
            ...
            <span className="text-sm">{capacity} {capacity === 1 ? tCommon('guest') : tCommon('guests')}</span>
          </div>
          <div className="flex items-center space-x-1">
        ...
        <div className="mb-6">
          <div className="flex flex-wrap gap-2">
            {features.slice(0, 3).map((feature, i) => (
              ...
              >
                {feature === 'Free WiFi' && <IoWifiOutline className="w-3 h-3" />}
                {feature === 'Parking' && <IoCarOutline className="w-3 h-3" />}
                {feature === 'Room service' && <IoRestaurantOutline className="w-3 h-3" />}
                <span>{feature}</span>
              </span>
            ))}
            {features.length > 3 && (
        ...
        {(roomType === 'corporate' || connectingAvailable) && (
          <div className="mt-4 pt-4 border-t border-gray-100">
            <div className="flex items-center justify-between text-xs">
              {roomType === 'corporate' && (
                <span className="text-accent-600 font-medium">{t('corporate_rental')}</span>
              )}
              {connectingAvailable && (
                <span className="text-green-600 font-medium">{t('family')} ✓</span>
              )}
            </div>
          </div>
        )}
