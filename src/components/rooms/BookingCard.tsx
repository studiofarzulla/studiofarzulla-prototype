...
export default function BookingCard({
  ...
  const isBookingDisabled =
    !isFormValid || availability === 'unavailable' || isBooking;

  return (
    <div
      className={`bg-white rounded-2xl shadow-soft p-6 sticky top-24 ${className}`}
    >
      {/* Header */}
      ...
      <div className='text-center mb-6 p-4 bg-primary-50 rounded-xl border border-primary-200'>
        <div className='text-3xl font-bold text-primary-600'>${basePrice}</div>
        <div className='text-sm text-gray-600'>{t('per_night')}</div>
      </div>

      {/* Availability Status */}
      <div
        className={`flex items-center justify-center p-3 rounded-lg border mb-6 ${status.bgColor}`}
      >
        ...
        >
          ...
          <div className='space-y-2 text-sm'>
            ...
            <div className='flex justify-between'>
              <span>Guests</span>
              <span>
                {guests} adults {children > 0 && `+ ${children} children`}
              </span>
            </div>
            {(() => {
              const nights =
